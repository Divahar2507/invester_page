...
export default function DashShell({ children }) {
  ...
  React.useEffect(() => {
    async function loadUser() {
      try {
        const user = await import("../api").then(m => m.api.me());
    ...
    <div className="dash">
      <aside className="dash-side">
        ...
        <nav className="dash-nav">
          <NavLink to="/dashboard" className={({ isActive }) => `dash-link ${isActive ? "active" : ""}`}>
            <span className="dash-link-ic"><Icon name="grid" /></span>
            Dashboard
          </NavLink>

          <NavLink to="/my-pitches" className={({ isActive }) => `dash-link ${isActive ? "active" : ""}`}>
            <span className="dash-link-ic"><Icon name="doc" /></span>
            My Pitches
          </NavLink>

          <NavLink to="/opportunities" className={({ isActive }) => `dash-link ${isActive ? "active" : ""}`}>
            <span className="dash-link-ic"><Icon name="spark" /></span>
            Opportunities
            <span className="pillCount">3</span>
          </NavLink>

          <NavLink to="/investors" className={({ isActive }) => `dash-link ${isActive ? "active" : ""}`}>
            <span className="dash-link-ic"><Icon name="users" /></span>
            Investors
          </NavLink>

          <NavLink to="/messages" className={({ isActive }) => `dash-link ${isActive ? "active" : ""}`}>
            <span className="dash-link-ic">
        ...
        <div className="dash-side-bottom">
          <NavLink to="/settings" className={({ isActive }) => `dash-link ${isActive ? "active" : ""}`}>
      ...
      <section className="dash-main">
        <header className="dash-top">
          ...
          <div className="dash-top-right">
            <button className="bellBtn" type="button" aria-label="Notifications" onClick={() => navigate("/notifications")}>
              <Icon name="bell" />
