...
def create_pitch(
    ...
    # Calculate amount_seeking for sorting
    amount_val = 0
    if pitch.raising_amount:
        clean_amount = pitch.raising_amount.replace('$', '').replace(',', '').strip()
        try:
             # Basic parsing: 100k, 1M, or 500000
...
def update_pitch(
    pitch_id: int,
    pitch_update: PitchCreate,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    if current_user.role != "startup":
        raise HTTPException(status_code=403, detail="Only startups can update pitches")
    
    db_pitch = db.query(Pitch).filter(Pitch.id == pitch_id).first()
    if not db_pitch:
        raise HTTPException(status_code=404, detail="Pitch not found")
        
    if db_pitch.startup.user_id != current_user.id:
    ...
    # Recalculate amount_seeking if raising_amount changes
    if 'raising_amount' in update_data:
        amount_val = 0
        r_amt = update_data['raising_amount']
        if r_amt:
            clean = r_amt.replace('$', '').replace(',', '').strip()
            try:
                if 'M' in clean:
                    amount_val = float(clean.replace('M', '')) * 1_000_000
                elif 'k' in clean.lower():
                    amount_val = float(clean.lower().replace('k', '')) * 1_000
                else:
                    amount_val = float(clean)
            except:
                amount_val = 0
        db_pitch.amount_seeking = int(amount_val)
